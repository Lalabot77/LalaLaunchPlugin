...
Validated against commit: a5bf955
...
- ShiftAssist.Beep restored as audio-issued pulse; ShiftLight exports now canonical; legacy aliases retained.
- Shift Assist shift light routing supports per-profile mode selection (Primary/Urgent/Both) while keeping legacy light aliases (`ShiftAssist.BeepLight`, `ShiftAssist.BeepPrimary`, `ShiftAssist.BeepUrgent`) mapped to canonical `ShiftAssist.ShiftLight*` outputs.
