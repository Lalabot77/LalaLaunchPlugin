...
- `LalaLaunch.PreRace.Stints`: key threshold signal (decimal, 1dp) computed from fuel requirement vs tank capacity; this replaces separate planned/calculated stop exports for dash contract use.
...
- `LalaLaunch.PreRace.FuelDelta`: single pre-race delta output. `Single Stop` uses current fuel + pit-menu refuel intent (`PitSvFuel`), while Auto/No Stop/Multi Stop use current fuel only.
- `LalaLaunch.PreRace.FuelSource` / `LapTimeSource`: short informational source labels (`planner`, `simhub`, `fallback`) for dash message assembly.
