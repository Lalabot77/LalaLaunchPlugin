...
### Checkpoint index semantics
- `PlayerCheckpointIndexNow` is the **current** checkpoint index every tick (`floor(lapPct * checkpoints)` when lapPct is valid).
- `PlayerCheckpointIndexCrossed` is only set **on crossing ticks** (index just crossed); `PlayerCheckpointCrossed` flags that event.
- `PlayerCheckpointIndex` remains a legacy alias for `PlayerCheckpointIndexCrossed`.
- RealGap now refreshes **every tick** using `PlayerCheckpointIndexNow` to keep gaps from going stale.
...
- Slot changes: `SlotCarIdxChangedThisTick` (count of slot CarIdx changes across ahead/behind slots)
