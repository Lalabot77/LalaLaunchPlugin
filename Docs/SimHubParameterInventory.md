...
| PitExit.Ahead.Name / CarNumber / ClassColor | string | Identity of the nearest same-class connected car predicted ahead after the stop (closest negative delta). Empty when none. | Per tick (Race only, lap gate ≥2). | `Opponents.cs` — pit-exit predictor scan + `AttachCore`【F:Opponents.cs†L532-L658】【F:LalaLaunch.cs†L3126-L3133】 |
| PitExit.Ahead.GapSec | double | Absolute seconds to the nearest predicted-ahead car after pit loss (from delta vs. player predicted gap). 0 when none. | Per tick (Race only, lap gate ≥2). | `Opponents.cs` — pit-exit predictor scan + `AttachCore`【F:Opponents.cs†L532-L658】【F:LalaLaunch.cs†L3126-L3133】 |
| PitExit.Behind.Name / CarNumber / ClassColor | string | Identity of the nearest same-class connected car predicted behind after the stop (closest positive delta). Empty when none. | Per tick (Race only, lap gate ≥2). | `Opponents.cs` — pit-exit predictor scan + `AttachCore`【F:Opponents.cs†L532-L676】【F:LalaLaunch.cs†L3130-L3137】 |
| PitExit.Behind.GapSec | double | Absolute seconds to the nearest predicted-behind car after pit loss (from delta vs. player predicted gap). 0 when none. | Per tick (Race only, lap gate ≥2). | `Opponents.cs` — pit-exit predictor scan + `AttachCore`【F:Opponents.cs†L532-L676】【F:LalaLaunch.cs†L3130-L3137】 |
...
| Pit.EntryDistanceToLine_m | double | Distance to pit entry line (iRacing extra distance; fallback pct×track length). Clamped 0–500 m; assist resets when ≥500 m. | Per tick when armed. | `PitEngine.UpdatePitEntryAssist` + `AttachCore`【F:PitEngine.cs†L279-L343】【F:LalaLaunch.cs†L2751-L2754】 |
| Pit.EntryRequiredDistance_m | double | Constant-decel distance needed to reach pit speed at the line using the profiled decel. | Per tick when armed. | `PitEngine.UpdatePitEntryAssist` + `AttachCore`【F:PitEngine.cs†L318-L343】【F:LalaLaunch.cs†L2752-L2754】 |
| Pit.EntryMargin_m | double | Distance remaining minus required distance; positive = early, negative = late. Primary dash signal. | Per tick when armed. | `PitEngine.UpdatePitEntryAssist` + `AttachCore`【F:PitEngine.cs†L323-L343】【F:LalaLaunch.cs†L2752-L2755】 |
