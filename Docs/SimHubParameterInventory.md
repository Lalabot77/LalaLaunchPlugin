...
| Exported name | Type | Units / meaning | Update cadence | Defined in |
| --- | --- | --- | --- | --- |
| Radio.TransmitShortName | string | Abbreviated name for the currently transmitting car (from `SessionData.DriverInfo.DriversXX.AbbrevName`), empty when no transmitter or missing. | Per tick. | `LalaLaunch.cs` transmit caching + `AttachCore`【F:LalaLaunch.cs†L3655-L3658】【F:LalaLaunch.cs†L7046-L7324】 |
| Radio.TransmitFrequencyName | string | Frequency name for the currently transmitting car (matched by radio index + `FrequencyNum`), empty when no transmitter or mapping missing. | Per tick. | `RadioFrequencyNameCache.cs` + `LalaLaunch.cs` transmit caching + `AttachCore`【F:RadioFrequencyNameCache.cs†L1-L345】【F:LalaLaunch.cs†L3655-L3658】【F:LalaLaunch.cs†L7046-L7324】 |
| Radio.TransmitFrequencyMuted | bool | Channel-level mute state for the currently transmitting frequency (from `FrequenciesYY.Muted`), false when no transmitter or mapping missing. This reflects the channel setting only (not per-driver mutes) and updates live if the channel mute toggles mid-transmission. | Per tick. | `RadioFrequencyNameCache.cs` + `LalaLaunch.cs` transmit caching + `AttachCore`【F:RadioFrequencyNameCache.cs†L1-L345】【F:LalaLaunch.cs†L3655-L3658】【F:LalaLaunch.cs†L7046-L7302】 |
| Radio.TransmitClassPosLabel | string | Class position label for the currently transmitting car, formatted as `{class} P{pos}` when class short name is available (falls back to `P{pos}` when not). Empty when no transmitter or identity missing. | Per tick. | `LalaLaunch.cs` transmit caching + `AttachCore`【F:LalaLaunch.cs†L3655-L3658】【F:LalaLaunch.cs†L7248-L7285】 |
